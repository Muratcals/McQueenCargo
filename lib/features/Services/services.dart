...
import 'package:mc_queen_cargo/features/Services/atomic_service_component.dart';
...
class Services {
  ...
  Future<LoginModel> login(Map<String, String> model, String? incoming) async {
    try {
      var result = await ServiceProcess.postService(
  ...
  Future<AddressModel> getCustomerAddress({int addressId = 1}) async {
    try {
      var response = await ServiceProcess.getService(
  ...
  Future<String> postCargo({
    required PostCourierModel model,
  }) async {
    try {
      await ServiceProcess.postService(
  ...
  Future<List<CargoInformationModel>> getAllCargoInformation({tcNo}) async {
    try {
      var response = await ServiceProcess.getService(
