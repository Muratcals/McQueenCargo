...
class _CustomerAddresslistItems extends StatelessWidget {
  const _CustomerAddresslistItems(
      {required this.item, required this.process, required this.setStatePage});
      ...
      onTap: () async {
        if (process.contains("getCourier")) {
          ...
          var result = await Get.toNamed(
              GenerateRoute.customerAddressInformationPage,
              arguments: {"addressId": item.id});
          if (result != null) {
            setStatePage(result as int);
          }
        }
      },
      child: Container(
...
class _ReceiverAddresslistItems extends StatelessWidget {
  const _ReceiverAddresslistItems(
      {required this.item, required this.process, required this.setStatePage});
    ...
    return InkWell(
      onTap: () async {
        if (process.contains("getCourier")) {
          ...
          var result = await Get.toNamed(
              GenerateRoute.receiverAddressInformationPage,
              arguments: {"addressId": item.id});
          if (result != null) {
            setStatePage(result);
          }
...
class _AppBarBottomItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: TextButton(
        onPressed: () async {
          await Get.toNamed(GenerateRoute.createAddressPage,
              arguments: incoming);
          reload();
        },
        child: Row(
