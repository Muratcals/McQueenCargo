      ...
      return AtomicOrangeButton(
        onPressed: () async {
          ...
          if (isValidate) {
            ...
            await service
                .login(model, controller.partnerController.accessToken.value,
                    "Login")
                .then((value) async {
              await saveDatabase(value.id!);
              EasyLoading.showToast("Giriş başarılı");
              controller.partnerController.currentAuth.value = value.id!;
  ...
  Widget build(BuildContext context) {
    return GetBuilder<LoginController>(builder: (controller) {
      return AtomicTextFormField(
        onSaved: (newValue) {
          controller.password = newValue!.trim();
        },
        obscureText: true,
        title: "Şifre",
        validator: (value) {
...
class _EPostaTextField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<LoginController>(builder: (controller) {
      return AtomicTextFormField(
        onSaved: (newValue) {
          controller.userName = newValue!.trim();
        },
