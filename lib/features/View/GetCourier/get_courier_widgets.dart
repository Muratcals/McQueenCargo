...
class _GetCourierBodyState extends State<_GetCourierBody> with GetCourierMixin {
  @override
  void dispose() async {
    await Get.delete<GetCourierController>();
    super.dispose();
  }
...
class _BottomContainer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return GetX<GetCourierController>(
      builder: (controller) {
        return controller.customerAddresssModel.value != null &&
                controller.receiverAddressModel.value != null
            ? Container(
                ...
                children: [
                  CarouselSlider(
                    ...
                    options: CarouselOptions(
                        ...
                        aspectRatio: 1.4,
                        initialPage: 0,
                        scrollPhysics: controller.selectedDate.value == 0
            ...
            children: [
              InkWell(
                onTap: () async {
                  ...
                  if (model != null) {
                    controller.customerAddresssModel.value = model;
                    controller.partnerController.getCourierModel.value
                            .customerMobilAdressId =
                        controller.customerAddresssModel.value?.id;
                  }
                },
                child: Container(
                  padding: CustomPadding.allInset(5),
                  child: _SelectedAddressColumnItem(
                      ...
                      title: controller.customerAddresssModel.value?.title ??
                          "Gönderici Adresi Seçiniz",
                      district:
                          controller.customerAddresssModel.value?.districtName,
                      provinceName:
                          controller.customerAddresssModel.value?.provinceName),
                ),
              ),
              SizedBox(height: 5.h),
              Divider(
              ...
              InkWell(
                onTap: () async {
                  ...
                  if (model != null) {
                    controller.receiverAddressModel.value = model;
                    controller.partnerController.getCourierModel.value
                        .receiverId = controller.receiverAddressModel.value?.id;
                  }
                },
                child: Container(
                  padding: CustomPadding.allInset(5),
                  child: _SelectedAddressColumnItem(
                      ...
                      title: controller.receiverAddressModel.value?.title ??
                          "Alıcı Adresi Seçiniz",
                      district:
                          controller.receiverAddressModel.value?.districtName,
                      provinceName:
                          controller.receiverAddressModel.value?.provinceName),
                ),
              ),
            ],
          ),
